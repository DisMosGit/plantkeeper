...
> на месте, README несёт финальную диаграмму. `make lint` чист, `make test` — 803
> теста, покрытие 96% (domain 100%, application 97%, infrastructure 96%).
> Коммиты созданы локально, push в `origin` не выполнялся.
...
- **Каталог строит админ-процесс, а воркер его читает.** Каталог и диаграмма потока
  — утверждения обо *обеих* сторонах платформы: без проекций события, которые читает
  только read-модель, выглядели бы как «нет консьюмера вообще». Импортировать Django
  для этого нельзя ни из `plantkeeper.infrastructure`, ни из воркерного генератора,
  поэтому `make contracts` запускает админ-генератор первым: он пишет
  `docs/asyncapi-read.json` и `docs/diagrams/`, а воркерный получает его документ
  через `--catalogue-from`. Порядок шагов в `tools/contracts.py` — часть контракта,
  а не деталь реализации.
- **Диаграмма потока отделяет «свой контекст» от «никто не читает».** Потребительские
  рёбра рисуются только между контекстами, поэтому под диаграммой две подписи: какие
  события читает только собственный контекст (`OnboardPlantSaga` для `PlantAdded`) и
  какие не читает никто (`SensorOffline`, четыре lifecycle-события саг). Молчание
  было бы неотличимо от «нет консьюмера».
