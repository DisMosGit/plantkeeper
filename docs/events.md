...
> **Status:** the catalogue below is exactly what `packages/domain` implements
> (`tests/unit/domain/test_events_catalogue.py` guards it), its transport is
> implemented in
...
> `docs/asyncapi-read.json`, generated by `make contracts` from the `EVENT_TOPICS`
> mapping and the consumer registries. Those JSON documents are gitignored build
> products, like the gRPC stubs; the diagrams generated beside them
> ([`docs/diagrams/event-flow.md`](diagrams/event-flow.md)) are committed.
